...
- Run the folowing command inside the Drupal docker container: `vendor/bin/drush cr`
...
We need to enable the Changebank module. To do so, navigate to /admin/modules and select the checkbox next to Changebank and then click on the `Install` button or run the following command inside the Drupal docker container:
...
Navigate to localhost/admin/config/people/openid-connect where we were earlier to create the client and once again click on the settings tab.
