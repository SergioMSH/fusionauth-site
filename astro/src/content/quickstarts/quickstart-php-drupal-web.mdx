...
Finally, we need to enable the Changebank module. To do so, navigate to /admin/modules and select the checkbox next to Changebank and then click on the `Install` button or run the following command inside the Drupal docker container:
