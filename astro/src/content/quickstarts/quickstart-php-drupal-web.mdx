...
Run through the wizard and fill in all required fields. Make a note to enter the database name as `changebank`, the database user as `drupal` and the database password as `verybadpassword`. Also, make sure to select the port number 3307 for the database host.
...
You might need to rebuild the docker containers and volumes for the changes to take effect. You can do so by running the following command:

```bash
docker compose down -v && docker compose up -d
```

Once the containers are up and running, we want to enter the drupal container so that we can run drush commands. To do so, run the following command:
...
In the browser add /user/logout to logout of the Drupal application. Navigate to /user/login and click on the `Login with generic` button.
...
- .../web/modules/custom/changebank/changebank.info.yml
- .../web/modules/custom/changebank/changebank.module
- .../web/modules/custom/changebank/changebank.routing.yml
- .../web/modules/custom/changebank/src/Controller/ChangebankController.php
- .../web/modules/custom/changebank/src/Plugin/Block/ExampleBlock.info.yml
```
...
- .../web/modules/custom/changebank/src/Plugin/Block/UserEmailBlock.php
```
